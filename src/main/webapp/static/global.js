...
// var ctxPath = 'http://127.0.0.1:8080';
...
//				var type = $(item)[0].type;
	...
	//				var feild_class = $(item).attr("class");
		...
		_inputReset : function(parentField){
			var inputs = $(parentField).find("input");
			var selects = $(parentField).find("select");
			var textareas = $(parentField).find("textarea");
			...
			$.each(inputs, function(i, item){
				...
				if(tagName == "INPUT" && (type == "text" || type == "password")
					...
					$('input[name="'+feild_name+'"]:first').attr("checked",true);
				}else if(tagName == "INPUT" && type == "checkbox"){

					$('input:checkbox[name="'+feild_name+'"]:first').attr(checked,true);
				}else{
					if(feild_id){
						//retData[feild_id] = ZRJC.getValue($(feild_id));	//其它情况 至获取value属性 
					}
...
//				var feild_id = $(item).attr("id");
//				var feild_name = $(item).attr("name");
				$("option:first", $(item)).prop("selected", 'selected');
...
//					+ " " + targetDate.getHours() + ":" +targetDate.getMinutes() + ":" + targetDate.getSeconds());
	...
	return {
		callRemote : methods._callRemote,
		...
		resetInput : methods._inputReset,
...
var ZRJC = {
		...
		resetInput : _ZRJC.resetInput,
		getCheckboxVal : _ZRJC.getCheckboxVal
		
};

ZRJC.downloadFile = function(action,params){
	   var curForm=$("#downloadFiles");
	   var $tempForm=$("<form style='display:none;'></form>");
	   for (var key in params) {  
           console.log("map["+key+"]"+params[key]);
